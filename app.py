...
<div style="background: linear-gradient(135deg, #d7263d, #a71d2a); padding: 30px; border-radius: 15px; text-align: center;">
    ...
    <p style="color: white; margin-top: 10px;">
        <span style="background: rgba(255,255,255,0.2); padding: 4px 15px; border-radius: 20px;">🏆 CodeStorm 2026</span>
        <span style="background: rgba(255,255,255,0.2); padding: 4px 15px; border-radius: 20px; margin-left: 10px;">🤖 AI-Powered</span>
    </p>
</div>
...
def find_donors(patient_lat, patient_lon, patient_blood, urgency='normal'):
    ...
    return compatible.head(5), len(compatible), "✅ Donors found!"
...
if st.button("🔍 Find Donors Now", use_container_width=True):
    ...
    donors, total, status = find_donors(lat, lon, blood_group, urgency.lower())
    
    if donors is None:
        st.error("❌ No compatible donors found! 🚨 Escalating to hospital network...")
    else:
        st.success(f"✅ {status} | {total} compatible donors found!")
...
tab1, tab2, tab3 = st.tabs(["Blood Group", "Donations", "Cities"])

with tab1:
    fig, ax = plt.subplots(figsize=(8, 4))
    df['blood_group'].value_counts().plot(kind='bar', ax=ax, color=['#e74c3c', '#3498db', '#2ecc71', '#f1c40f'])
...
with tab2:
    fig, ax = plt.subplots(figsize=(8, 4))
    df['donations'].hist(bins=10, color='#2ecc71', edgecolor='black', alpha=0.7)
...
with tab3:
    fig, ax = plt.subplots(figsize=(8, 4))
    df['city'].value_counts().plot(kind='barh', ax=ax, color='#e67e22')
